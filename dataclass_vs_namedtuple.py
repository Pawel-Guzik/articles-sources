...
dataclass_create = timeit.repeat(stmt="ExampleDataclass()",
...
dataclass_read = timeit.repeat(stmt="dataclass_instance.foo",
...
dataclass_with_attr = timeit.repeat(stmt="ExampleDataclass('some_string_value')",
...
nt_collections_create = timeit.repeat(stmt="CollectionsNamedTuple()",
...
nt_collections_read = timeit.repeat(stmt="tuple_instance.foo",
...
nt_collections_with_attr = timeit.repeat(stmt="CollectionsNamedTuple('some_string_value')",
...
nt_typing_create = timeit.repeat(stmt="TypingNamedTuple()",
...
class TypingNamedTuple(NamedTuple):
    foo: str
"""
nt_typing_with_attr = timeit.repeat(stmt="TypingNamedTuple('some_string_value')",
...
nt_typing_read = timeit.repeat(stmt="tuple_instance.foo",
...
results = PrettyTable(
    ["", "dataclass", "typing NamedTuple", "collections namedtuple"])

results.add_row(["Create empty object",
                 f"{format_result(min(dataclass_create))}",
                 f"{format_result(min(nt_typing_create))}",
                 f"{format_result(min(nt_collections_create))}"])

results.add_row(["Create object with attr",
